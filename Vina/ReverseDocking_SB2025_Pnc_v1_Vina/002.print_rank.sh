...
#SBATCH --nodes=1
#SBATCH --ntasks=2
#SBATCH --job-name=vina_RD_002
#SBATCH --output=vina_RD_002.out
...
for lig_sys in `cat $work_dir/FARMA.systems.all`;do
  ...
  val=`sort -n -k5  tmp_scores_fam.txt| grep $best_pair | awk '{print $5}'`
...
for lig_sys in `cat $work_dir/FARMA.systems.all`;do
  ...
  val=`sort -n -k5  tmp_scores_class.txt| grep $best_pair | awk '{print $5}'`
